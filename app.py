...
def get_bot_response(user_message, model_name):
    ...
    try:
        ...
        payload = {
            ...
            "prompt": user_message,
            "stream": False,
            "system": "Keep answers concise and to the point; use bullet points instead of long paragraphs."
        }
...
for message in st.session_state.messages:
    if message["role"] == "user":
        ...
        with col2:
            st.info(message["content"].replace("\n", "  \n"))
        st.markdown(
            f"<div style='margin-top: -10px; margin-bottom: 24px; font-size: 0.9em; text-align: right;'>"
            f"<b>You</b> ({message['timestamp']})</div>",
            unsafe_allow_html=True
        )
    else:
        col1, col2 = st.columns([0.9, 0.1])
        with col1:
            st.success(message["content"].replace("\n", "  \n"))
        st.markdown(
            f"<div style='margin-top: -10px; margin-bottom: 24px; font-size: 0.9em;'>"
            f"<b>Bot</b> ({message['timestamp']})</div>",
            unsafe_allow_html=True
        )

# Show typing indicator with reserved space
with st.container():
    col1, col2 = st.columns([0.9, 0.1])
    with col1:
        if st.session_state.is_typing:
            st.warning("🤖 Thinking...")
            st.markdown(
                f"<div style='margin-bottom: 70px;'>",
                unsafe_allow_html=True
            )
        else:
...
if clear_button:
    st.session_state.messages = [{
        "role": "assistant",
        "content": "👋 Hello! I'm your local ChatGPT. Ask me anything!",
        "timestamp": datetime.now().strftime("%H:%M:%S")
    }]
    st.session_state.is_typing = False
    st.rerun()
...
with st.sidebar:
    ...
    if st.session_state.available_models:
        ...
        st.info(f"{models_count} models available")
